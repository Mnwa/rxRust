...
///
pub fn from_future<F, Item>(f: F) -> ObservableBase<SharedOp<FutureEmitter<F>>>
where
  F: Future<Output = Item> + Send + Clone + Sync + 'static,
{
  ObservableBase::new(SharedOp(FutureEmitter(f)))
}

#[derive(Clone)]
pub struct FutureEmitter<F>(F);

impl<Item, F> SharedEmitter for SharedOp<FutureEmitter<F>>
where
  F: Future<Output = Item> + Send + Sync + 'static,
{
  ...
  {
    let fmapped = (self.0).0.map(move |v| of::OfEmitter(v).emit(subscriber));
...
pub fn from_future_result<F, Item, Err>(
  f: F,
) -> ObservableBase<SharedOp<FutureResultEmitter<F, Item, Err>>>
where
...
{
  ObservableBase::new(SharedOp(FutureResultEmitter(f, PhantomData)))
}

#[derive(Clone)]
pub struct FutureResultEmitter<F, Item, Err>(F, PhantomData<(Item, Err)>);

impl<Item, Err, F> SharedEmitter for SharedOp<FutureResultEmitter<F, Item, Err>>
where
  Item: Send + Sync + 'static,
  Err: Send + Sync + 'static,
  F: Future + Send + Clone + Sync + 'static,
  <F as Future>::Output: Into<Result<Item, Err>>,
{
    ...
    O: Observer<Self::Item, Self::Err> + Send + Sync + 'static,
  {
    let fmapped = (self.0)
      .0
      .map(move |v| of::ResultEmitter(v.into()).emit(subscriber));
    DEFAULT_RUNTIME.lock().unwrap().spawn(fmapped).unwrap();
  }
}
...
fn smoke() {
  ...
  let c_res = res.clone();
  {
    from_future_result::<_, _, ()>(future::ok(1)).subscribe(move |v| {
      *res.lock().unwrap() = v;
    });
